        ...
        if let source = request.content.userInfo["source"] as? String, source == "webengage" {
            ...
            if let expandableDetails = request.content.userInfo["expandableDetails"] as? [String: Any], let style = expandableDetails["style"] as? String {
                if style == "CAROUSEL_V1", let items = expandableDetails["items"] as? [[String: Any]] {
                    WERenderer.drawMultiImageView(with: items, bestAttemptContent: self.bestAttemptContent, contentHandler: self.contentHandler)
                } else if style == "TILES", let items = expandableDetails["tiles"] as? [[String: Any]] {
                    let backgroundImage = expandableDetails["image"] as? String
                    registerCategoryAndExecute {
                        WERenderer.drawMultiImageView(with: items, backgroundImage: backgroundImage, bestAttemptContent: self.bestAttemptContent, contentHandler: self.contentHandler)
                    }
